	...
	foreach ($search_fields AS $search_field) {
		if (in_array($search_field->content_field->type, array('datetime'))) {
			...
			if ($search_field->content_field->hide_past_dates) {
				...
				$query = "SELECT DISTINCT meta1.post_id FROM {$wpdb->postmeta} AS meta1 WHERE ((meta1.meta_key = '_content_field_" . $id . "_date_end' AND (CAST(meta1.meta_value AS SIGNED) <= " . time() . ")))";
