...
function w2dc_getPagesWithShortcode($shortcode) {
	...
	$flat_pages = $wpdb->get_results("SELECT ID AS id, post_name AS slug FROM {$wpdb->posts} WHERE (post_content LIKE '%[" . $shortcode . "]%') AND post_status = 'publish' AND post_type = 'page'", ARRAY_A);
	$custom_pages = $wpdb->get_results("SELECT ID AS id, post_name AS slug FROM {$wpdb->posts} WHERE (post_content LIKE '%[" . $shortcode . " %') AND post_status = 'publish' AND post_type = 'page'", ARRAY_A);
